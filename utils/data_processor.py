...
class DataProcessor:
    ...
    async def process_speaker_list(self, input_file: str, output_file: str, max_speakers: Optional[int] = None) -> None:
        ...
        try:
            ...
            if len(speakers) > max_speakers:
                logger.warning(f"Limiting processing to {max_speakers} speakers (out of {len(speakers)} total)")
    ...
    def _read_csv_file(self, input_file: str) -> List[Speaker]:
        ...
        name_col = self._find_column(df, ['name', 'Name', 'speaker_name', 'Speaker Name'])
        title_col = self._find_column(df, ['title', 'Title', 'speaker_title', 'Speaker Title', 'job_title'])
        company_col = self._find_column(df, ['company', 'Company', 'speaker_company', 'Speaker Company'])
        ...
        df_clean = df.drop_duplicates(subset=[name_col, title_col, company_col], keep='first')
        
        if len(df) != len(df_clean):
            logger.warning(f"Removed {len(df) - len(df_clean)} duplicate entries from CSV")
    ...
    def _parse_csv_data(self, df: pd.DataFrame) -> List[Speaker]:
        ...
        name_col = self._find_column(df, ['name', 'speaker', 'speaker_name', 'full_name'])
        title_col = self._find_column(df, ['title', 'job_title', 'role', 'position'])
        company_col = self._find_column(df, ['company', 'organization', 'firm', 'employer'])
    ...
    def _write_output(self, speakers: List[Speaker], output_file: str) -> None:
        ...
        for speaker in speakers:
            data.append({
                ...
                'Company Category': speaker.company_category.value if speaker.company_category else 'Unknown',
    ...
    def _print_summary(self, speakers: List[Speaker]) -> None:
        ...
        categories = {}
        for speaker in speakers:
            category = speaker.company_category.value if speaker.company_category else 'Unknown'
        ...
        emails_generated = sum(1 for s in speakers if s.company_category != CompanyCategory.COMPETITOR)
        logger.info(f"\nEmails generated: {emails_generated}")
        logger.info(f"Competitors excluded: {categories.get(CompanyCategory.COMPETITOR.value, 0)}")
