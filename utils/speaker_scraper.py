...
class SpeakerScraper:
    ...
    def scrape_website(self, url: str = None) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.info(f"Successfully fetched website (Status: {response.status_code})")
    ...
    def _parse_title_company(self, title_company_text: str, name: str) -> Optional[Dict[str, str]]:
    ...
    def _remove_duplicates(self, speakers: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    def _clean_company_name(self, company: str) -> str:
        ...
        company = re.sub(r'\s+(Ltd|LLC|Inc|Corp|Limited|Corporation|Company|Co)\.?$', '', company, flags=re.IGNORECASE)
    ...
    def save_to_csv(self, speakers: List[Dict[str, str]], output_file: str) -> None:
        ...
        try:
            ...
            unique_speakers = self._remove_duplicates(speakers)
            logger.info(f"Removed {len(speakers) - len(unique_speakers)} duplicate entries")
            ...
            logger.info(f"Saved {len(unique_speakers)} unique speakers to {output_file}")
...
def main():
    ...
    try:
        ...
        for i, speaker in enumerate(speakers[:5]):
            logger.info(f"{i+1}. {speaker['name']} - {speaker['title']} at {speaker['company']}")
