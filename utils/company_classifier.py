...
class CompanyClassifier:
    ...
    def _save_cache(self) -> None:
        ...
        try:
            ...
            logger.info(f"Saved {len(self.classification_cache)} company classifications to cache")
        except Exception as e:
            logger.error(f"Error saving cache: {e}")
    
    def _get_cached_classification(self, company_name: str) -> Optional[CompanyCategory]:
        ...
        if normalized_name in self.classification_cache:
            ...
            try:
                ...
                logger.warning(f"Invalid cached category '{category_value}' for '{company_name}'")
                return None
        return None
    
    def _cache_classification(self, company_name: str, category: CompanyCategory) -> None:
    ...
    async def classify_company(self, company_name: str) -> CompanyCategory:
        ...
        try:
            ...
            if cached_category:
                logger.info(f"Using cached classification for '{company_name}': {cached_category.value}")
    ...
    async def _classify_company_with_research(self, company_name: str, company_info: str) -> CompanyCategory:
        ...
        try:
            ...
            response = await self.client.chat.completions.parse(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "You are an expert at classifying companies. Respond with a valid JSON object containing the category, confidence, and reasoning."},
            ...
            logger.info(f"AI classified '{company_name}' as {final_category.value} (confidence: {confidence:.2f})")
